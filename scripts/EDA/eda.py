...
def create_df_percentage(df, groupby_columns, percentage_column):
    ...
    df_percentage = df_percentage.rename(
        {percentage_column: "Percentage"}, axis="columns"
    )
...
def create_bar_plot(df, x, **kwargs):
    ...
    if "percentage" in list(kwargs.keys()):
        ...
        df = create_df_percentage(
            df=df, groupby_columns=columns, percentage_column=percentage_col
        )
        if "color" in list(kwargs.keys()):
            fig = px.bar(df, x=df[x], y=df["Percentage"], **kwargs)
        else:
            fig = px.bar(
                df, x=df[x], y=df["Percentage"], color=percentage_col, **kwargs
            )
    else:
...
def create_3d_scatter_plot(df, x, y, z, **kwargs):
    ...
    camera = dict(
        up=dict(x=0, y=1, z=0), center=dict(x=0, y=0, z=0), eye=dict(x=0, y=0, z=2.5)
    )
    fig.update_layout(
        scene=dict(
...
def visualisatie_opstapplaats(df):
    ...
    """
    print(
        "Het aantal overleefde/overleden passagiers ten opzichte van opstapplaats en geslacht."
    )
...
def visualisatie_leeftijd(df):
    ...
    """
    print(
        "Scatterplot van de overleefde/overleden passagiers ten opzichte van leeftijd."
    )
    create_scatter_plot(
        df=df,
        x="Passagier_Id",
...
def visualisatie_leeftijd_geslacht(df):
    ...
    """
    print(
        "3D scatterplot van de overleefde/overleden passagiers ten opzichte van leeftijd en geslacht."
    )
...
def visualisatie_familieleden(df):
    ...
    create_bar_plot(
        df=df,
        x="Totaal_aantal_familieleden",
        percentage="Overleefd",
        color_discrete_map=color_discrete_map,
        category_orders=category_orders,
        width=width,
    )
...
def basis_feiten(df):
    ...
    print(
        f"Er zitten {len(passengers)} passagiers in de dataset, daarvan heeft {rate_survivors:.2f}% het overleefd."
    )
